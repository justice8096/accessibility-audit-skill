...
function stringifyYaml(obj: unknown, indent: number = 2): string {
  ...
  function stringify(val: unknown, depth: number = 0): string {
    if (depth > MAX_YAML_DEPTH) throw new Error("YAML nesting depth exceeded");
    const spaces = " ".repeat(depth * indent);
    if (val === null || val === undefined) return "null";
    if (typeof val === "string") {
      const escaped = val.replace(/\\/g, "\\\\").replace(/"/g, '\\"').replace(/\n/g, "\\n").replace(/\r/g, "\\r").replace(/\t/g, "\\t").replace(/:/g, "\\:").replace(/#/g, "\\#").replace(/-/g, "\\-").replace(/\?/g, "\\?").replace(/\[/g, "\\[").replace(/\]/g, "\\]").replace(/\{/g, "\\{").replace(/\}/g, "\\}").replace(/,/g, "\\,").replace(/&/g, "\\&").replace(/\*/g, "\\*").replace(/!/g, "\\!").replace(/\|/g, "\\|").replace(/>/g, "\\>").replace(/%/g, "\\%").replace(/@/g, "\\@").replace(/`/g, "\\`").replace(/'/g, "\\'");
      totalSize += escaped.length;
      if (totalSize > MAX_YAML_SIZE) throw new Error("YAML size limit exceeded");
...
// --- Security: Input Sanitization (CWE-95, CWE-22) ---
const SAFE_IDENTIFIER = /^[a-z][a-z0-9_-]*$/i;
const SAFE_SKILL_ID = /^[a-z0-9-]+$/;
const MAX_YAML_DEPTH = 10;
const MAX_YAML_SIZE = 5 * 1024 * 1024;

function sanitizeIdentifier(input: string, context: string): string {
  if (!SAFE_IDENTIFIER.test(input)) {
    throw new Error("Invalid identifier in " + context + ": " + input);
  }
  return input;
}
...
function sanitizeSkillId(id: string): string {
  if (!SAFE_SKILL_ID.test(id)) {
    throw new Error("Invalid skill/command ID: " + id);
  }
  return id;
}
...
function loadManifest(): Manifest {
  ...
  for (const cmd of raw.commands) {
    sanitizeIdentifier(cmd.name, "command.name");
    for (const paramName of Object.keys(cmd.parameters?.properties || {})) {
      sanitizeIdentifier(paramName, "parameter.name");
    }
  }
  for (const skill of raw.skills) {
    sanitizeIdentifier(skill.name, "skill.name");
  }
...
function generateMcpServer(manifest: Manifest): void {
  ...
  for (const cmd of manifest.commands) {
    ...
    for (const [paramName, paramDef] of Object.entries(cmd.parameters.properties)) {
      ...
      let jsProp = "      " + paramName + ': { type: "' + paramDef.type + '", description: "' + escapeStringLiteral(paramDef.description) + '" }';
    ...
    const toolCode = ['import { z } from "zod";', 'import { loadSkillContent } from "../knowledge/loader.js";', "",
      ...
      "export const " + toolName + 'Definition = { name: "' + toolName + '", description: "' + escapeStringLiteral(cmd.description) + '",',
      '  inputSchema: { type: "object" as const, properties: {', ...jsonSchemaProps.map((p) => p.replace(/: "/g, ': "').replace(cmd.description.replace(/"/g, '\\"'), escapeStringLiteral(cmd.description)) + ","),
      ...
      '  const skillContent = await loadSkillContent("' + escapeStringLiteral(cmd.name) + '");',
      '  return JSON.stringify({ status: "success", command: "' + escapeStringLiteral(cmd.name) + '", message: "Tool executed.", skillPreview: skillContent.slice(0, 200), input: validated }, null, 2);',
      "}"].join("\n");
  ...
  const loaderCode = ['import { readFileSync } from "fs";', 'import { resolve, dirname } from "path";', 'import { fileURLToPath } from "url";', "",
    ...
    "const SAFE_ID = /^[a-z0-9-]+$/;", "const MAX_CACHE_SIZE = 50;",
    "const CACHE = new Map<string, string>();", "let cacheOrder: string[] = [];", "",
    "function sanitizeForLog(str: string): string { return str.replace(/[^a-z0-9._-]/gi, \"_\"); }",
    "function validateId(id: string): void {", "  if (!SAFE_ID.test(id)) throw new Error(\"Invalid ID: \" + sanitizeForLog(id));", "}",
    "export async function loadSkillContent(skillId: string): Promise<string> {",
    "  validateId(skillId);", '  if (CACHE.has(skillId)) return CACHE.get(skillId)!;', "  try {",
    '    const baseDir = resolve(__dirname, "../../knowledge/skills");', '    const p = resolve(baseDir, skillId + ".md");',
    '    if (!p.startsWith(baseDir)) throw new Error("Path traversal attempt");',
    '    const content = readFileSync(p, "utf-8");', "    if (CACHE.size >= MAX_CACHE_SIZE) { const oldest = cacheOrder.shift(); if (oldest) CACHE.delete(oldest); }",
    "    CACHE.set(skillId, content); cacheOrder.push(skillId); return content;",
    '  } catch (e) { console.error("Error loading skill " + sanitizeForLog(skillId) + ": " + (e instanceof Error ? e.message : "Unknown error")); return ""; }', "}",  "",
    "export async function loadCommandContent(commandId: string): Promise<string> {",
    "  validateId(commandId);", "  try {",
    '    const baseDir = resolve(__dirname, "../../knowledge/commands");', '    const p = resolve(baseDir, commandId + ".md");',
    '    if (!p.startsWith(baseDir)) throw new Error("Path traversal attempt");',
    '    return readFileSync(p, "utf-8");',
    '  } catch (e) { console.error("Error loading command " + sanitizeForLog(commandId) + ": " + (e instanceof Error ? e.message : "Unknown error")); return ""; }', "}"].join("\n");
  ...
  const indexCode = ['import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";',
    ...
    "function sanitizeToolName(name: string): string { return name.replace(/[^a-z0-9._-]/gi, \"_\"); }",
    ...
    "    try {", "      const { name, arguments: args = {} } = request.params;", "      switch (name) {",
    ...toolHandlerCases, '      default: throw new Error("Unknown tool: " + sanitizeToolName(name));', "      }", "    } catch (error) {",
    '      const errorMsg = error instanceof Error ? error.message : "Unknown error";', '      return { content: [{ type: "text", text: JSON.stringify({ error: errorMsg }, null, 2) }] };', "    }", "  });",
    "  await server.connect(transport);",
    '  console.error("' + escapeStringLiteral(manifest.metadata.name) + ' MCP Server running on stdio");', "}",  "",
